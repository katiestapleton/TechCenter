...
--Create the Customers, Orders, and RMA tables. Review the ERD
...
--update customers and orders tables with additional data
...
--delete all entries from RMA where the reason is Rejected or Returned
DELETE FROM RMA
WHERE Reason LIKE 'Re%';

--how many orders are from customers in the city of Framingham?
...
--What are the 3 most ordered products in the Southeastern region? See paper for regions description.
...
--What are the top 5 products with completed orders?
...
--What are the 3 most returned products in the Northwest region? See paper for regions description.
SELECT count(*) AS Returns, SKU, Description 
FROM Orders 
